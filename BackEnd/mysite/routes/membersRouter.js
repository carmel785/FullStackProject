...
router.get('/:user', function(req, res, next) {
  res.render('Subscriptions', {user: req.params.user});
});
       ...
       //check if the movie already watched so it wont show it again in the list:
        ...
        if(watchList!= 0)
        {
          ...
          if(req.session.memberDetails != undefined)
          {
            ...
            if(req.session.watchListMembers)
            {
              ...
              console.log("members array: "+req.session.memberDetails)
                res.render('AllMembers', {members, memberWatchList : req.session.memberDetails, movies: movies.data, moviesList: req.session.memberMoviesArray, subscriptions, watchListMembers : req.session.watchListMembers, user: req.params.user })
              }
              else
              {
                console.log("thats 6")
          ...
          else
          {
            ...
            res.render('AllMembers', {members , memberWatchList: req.session.memberDetails, movies: movies.data, subscriptions, user: req.params.user });
          }
        }
        else
        {
          ...
          res.render('AllMembers', {members, movies: movies1.data, moviesList: req.session.memberMoviesArray, subscriptions, user: req.params.user});
        }
    }
    else
    {
...
router.get('/addMember/:user', function(req, res, next) {
  res.render('AddMember', {user: req.params.user});
});
